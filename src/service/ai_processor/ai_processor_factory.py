...
def ai_processor_factory(config: dict) -> AiProcessor:
    ...
    if strategy == SummarizationStrategy.MAP_REDUCE:
        return _get_map_reduce_processor(config, logging_service, api_key, base_url, timeout, concurrency_limit)
...
def _get_linear_processor(config: dict, logging_service, api_key, base_url, timeout,
    ...
    return LinearAiProcessor(logging_service, system_prompt, user_prompt, model_name, temperature, max_tokens, top_p,
                             api_key,
