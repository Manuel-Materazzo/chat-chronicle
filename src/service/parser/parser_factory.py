...
def parser_factory(fileType: InputFileType, config: dict) -> Parser:
    ...
    if fileType == InputFileType.INSTAGRAM_EXPORT:
        return InstagramExport(chat_sessions_enabled, sleep_window_start, sleep_window_end,
                               ignore_chat_enabled, ignore_chat_before, ignore_chat_after)
    elif fileType == InputFileType.WHATSAPP_EXPORT:
        return WhatsappExport(chat_sessions_enabled, sleep_window_start, sleep_window_end,
                              ignore_chat_enabled, ignore_chat_before, ignore_chat_after)
